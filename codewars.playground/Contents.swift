...
func splitAddresses(from inputString: String) -> [String] {
    let addressPattern = #"(\b\d+\s+[A-Za-z0-9\s,]+,\s+[A-Za-z\s]+,\s+CA\s+\d+\b)"#
    ...
    do {
        ...
        for match in matches {
            ...
            let startIndex = inputString.index(inputString.startIndex, offsetBy: matchRange.lowerBound)
            let endIndex = inputString.index(inputString.startIndex, offsetBy: matchRange.upperBound)

            // Include any text between the previous match end and the current match start
            let nonAddressPart = String(inputString[inputString.index(inputString.startIndex, offsetBy: previousEnd)..<startIndex])
            if !nonAddressPart.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                addresses.append(nonAddressPart.trimmingCharacters(in: .whitespacesAndNewlines))
            }

            // Include the current match (address)
            let addressPart = String(inputString[startIndex..<endIndex])
            addresses.append(addressPart.trimmingCharacters(in: .whitespacesAndNewlines))

            // Update the previous match end
            previousEnd = matchRange.upperBound
        }

        // Include any text after the last match
        let remainingPart = String(inputString[inputString.index(inputString.startIndex, offsetBy: previousEnd)...])
        if !remainingPart.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            addresses.append(remainingPart.trimmingCharacters(in: .whitespacesAndNewlines))
        }
...
func splitStringOnZipCode(_ inputString: String, zipCode: String) -> [String] {
    let separator = zipCode.trimmingCharacters(in: .whitespacesAndNewlines)
    
    // Use components(separatedBy:) to split the string based on the zip code
    let components = inputString.components(separatedBy: separator)
    
    // Remove empty components
    let nonEmptyComponents = components.filter { !$0.isEmpty }
    
    // Add the zip code to each non-empty component
    let result = nonEmptyComponents.map { $0.trimmingCharacters(in: .whitespacesAndNewlines) + " " + zipCode }
    
    return result
}

print(test1)
//print(splitStringOnZipCode(test1, zipCode: "92106"))
